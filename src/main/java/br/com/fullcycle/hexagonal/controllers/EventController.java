...
public class EventController {
    ...
    public ResponseEntity<?> create(@RequestBody EventDTO input) {
        try {
            final var partnerId = Objects.requireNonNull(input.getPartner(), "Partner is required").getId();
            final var output = new CreateEventUseCase(eventService, partnerService)
                    .execute(new CreateEventUseCase.Input(input.getDate(), input.getName(), input.getTotalSpots(), partnerId));
    ...
    public ResponseEntity<?> subscribe(@PathVariable Long id, @RequestBody SubscribeDTO input) {
        try {
            final var output = new SubscribeCustomerToEventUseCase(eventService, customerService)
                                .execute(new SubscribeCustomerToEventUseCase.Input(id, input.getCustomerId()));
